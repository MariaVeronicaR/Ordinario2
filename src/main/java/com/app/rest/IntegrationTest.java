...
public class IntegrationTest 
{
  ...
  @Test
  public void givenUserExists_whenRequested_thenDataMatches () throws ClientProtocolException, IOException
  {
	     final HttpUriRequest request = new HttpGet("https://reqres.in/api/users/" + 5);

	      // When
	      final HttpResponse httpResponse = HttpClientBuilder.create().build().execute(request);
	      final User user = Mapper.retrieveResourceFromRootResponse(httpResponse, User.class); 
	      assertThat("Charles", is(user.getFirst_name()));
  }
}
*/